...
-- Raid target icon texture coordinates (2 columns x 4 rows)
-- Texture: Interface\TargetingFrame\UI-RaidTargetingIcons
-- Layout: Row0:[Star|Circle], Row1:[Diamond|Triangle], Row2:[Moon|Square], Row3:[Cross|Skull]
local RAID_TARGET_ICON_COORDS = {
	{0, 0.5, 0, 0.25},		-- [1] Star (left col, row 0)
	{0.5, 1, 0, 0.25},		-- [2] Circle (right col, row 0)
	{0, 0.5, 0.25, 0.5},	-- [3] Diamond (left col, row 1)
	{0.5, 1, 0.25, 0.5},	-- [4] Triangle (right col, row 1)
	{0, 0.5, 0.5, 0.75},	-- [5] Moon (left col, row 2)
	{0.5, 1, 0.5, 0.75},	-- [6] Square (right col, row 2)
	{0, 0.5, 0.75, 1},		-- [7] Cross (left col, row 3)
	{0.5, 1, 0.75, 1},		-- [8] Skull (right col, row 3)
};
...
function BananaBar3AssistButton:init(addon,name)
	...
	SecureActionQueue:FrameSetAttribute(self.frame,"type2", "menu")
		...
		self.frame:Hide()
		--self.Addon:Debug("init "..self.FrameName..self.Unit.."target");
		--self.Adoon:Debug("set b"..self.Unit.."target");
		...
		if BananaBarButtonNameCounter < 11 then
			self.visible = true;
			self.Addon:Debug("Visible");
			else
			self.Addon:Debug("Invisible");
			self.visible = false;
			self.frame:Hide()
			self.Addon:Debug("HideFrame");
		end
		self.Addon:Debug(BananaBarButtonNameCounter);
